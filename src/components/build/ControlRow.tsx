...
export default function ControlRow({ control, onChanged }: Props) {
  ...
  const [localQuestions, setLocalQuestions] = useState<any[]>(control.questions || []);
  ...
  // Sync local questions when parent re-fetches
  useEffect(() => {
    setLocalQuestions(control.questions || []);
  }, [control.questions]);

  const isEditValid = editCode.trim().length > 0 && editTitle.trim().length > 0;

  // DELETE CONTROL
  const handleDeleteControl = async () => {
    const ok = window.confirm(`Delete control "${control.code} — ${control.title}"?`);
    if (!ok) return;
    try {
  ...
  const handleUpdateControl = async () => {
    ...
    try {
      ...
      await apiClient.put(basePath, {
        ...
        riskWeight: Number(editRiskWeight) > 0 ? Number(editRiskWeight) : 1,
      });
      setShowEdit(false);
      onChanged();
    } catch (err: any) {
  ...
  // ADD QUESTION — local only, no remount
  const handleAddQuestion = async () => {
    if (!questionText.trim()) return;
    try {
      setLoadingQuestion(true);
      const res = await apiClient.post(`${basePath}/questions`, {
        text: questionText.trim(),
      });
      setLocalQuestions(prev => [...prev, res.data]);
      setQuestionText("");
  ...
  // UPDATE QUESTION — local only, no remount
  ...
  // DELETE QUESTION — local only, no remount
  const handleDeleteQuestion = async () => {
    if (!deletingQuestionId) return;
    try {
      setLoadingDeleteQuestion(true);
      await apiClient.delete(`${basePath}/questions/${deletingQuestionId}`);
      setLocalQuestions(prev => prev.filter(q => q.id !== deletingQuestionId));
    ...
    <div className="bg-slate-50 border border-slate-200 rounded-lg">
      ...
      <div className="px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={() => { setShowAddQuestion(true); setOpen(true); }}
        ...
        <div className="border-t px-4 py-3 bg-white">
          <div className="grid md:grid-cols-3 gap-3">
            ...
            <input
              ...
              onChange={(e) => setEditRiskWeight(Number(e.target.value))}
              className="border rounded-md px-3 py-2 text-sm"
            />
          </div>
                    ...
                    {isEditing && (
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            {loadingQuestionUpdate ? "Saving..." : "Save"}
                          </button>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="space-y-2 text-red-600">
                        ...
                        <div className="flex gap-2 mt-1">
                          <button
                            ...
                            className="px-3 py-1 border rounded text-sm text-slate-700"
                          >
                            Cancel
                          </button>
                          <button
                          ...
                          >
                            {loadingDeleteQuestion ? "Deleting..." : "Delete"}
                          </button>
                        </div>
                      </div>
                    )}
